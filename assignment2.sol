...
// You can edit this contract as much as you want. A template is provided here and you can change the function names and implement anything else you want, but the basic tasks mentioned here should be accomplished.
    ...
    function multidiv(uint x, uint y, uint z) internal pure returns(uint){
        return x*y/z;
    }
    function getCompoundInterest(uint256 principle, uint rate, uint time) public pure returns(uint256) {
            uint temp = principle;
            for(uint i =0; i < time;++i){
              principle += multidiv(principle,rate,100);
            }
    ...
    function viewDues() isOwner() public view returns(uint){
        return  loans[Owner];
    }
    function settleDues() isOwner() public view returns(bool para){
        if(balances[Owner] >= loans[Owner])
        {
    ...
    // function settleDues() isOwner() public view returns 
